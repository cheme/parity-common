...
/// use memorydb::*;
/// fn main() {
///   let mut m = MemoryDB::<KeccakHasher, RlpMemDbConf>::new();
...
impl<M> MemoryDB<M> where M: MemoryDBConf, <M::H as KeyHasher>::Out: HeapSizeOf {
	/// Create a new instance of the memory DB.
	pub fn new() -> MemoryDB<M> {
		MemoryDB {
			data: FastMap::<M::H,_>::default(),
			hashed_null_node: M::H::hash(M::NULL_VALUE)
		}
	}
	...
	///
	/// fn main() {
	///   let mut m = MemoryDB::<KeccakHasher>::new(&rlp::NULL_RLP);
	...
	pub fn drain(&mut self) -> FastMap<M::H, (DBValue, i32)> {
		mem::replace(&mut self.data, FastMap::<M::H,_>::default())
	}
	...
	/// when the refs > 0.
	pub fn raw(&self, key: &<M::H as KeyHasher>::Out) -> Option<(DBValue, i32)> {
		if key == &self.hashed_null_node {
			return Some((DBValue::from_slice(M::NULL_VALUE), 1));
		}
	...
	pub fn remove_and_purge(&mut self, key: &<M::H as KeyHasher>::Out) -> Option<DBValue> {
		if key == &self.hashed_null_node {
			return None;
		}
...
impl<M: MemoryDBConf> HashDB<M::H> for MemoryDB<M> {

	fn keys(&self) -> HashMap<<M::H as KeyHasher>::Out, i32> {
		self.data.iter()
	...
	fn get(&self, key: &<M::H as KeyHasher>::Out) -> Option<DBValue> {
		if key == &self.hashed_null_node {
			return Some(DBValue::from_slice(M::NULL_VALUE));
		}
	...
	fn contains(&self, key: &<M::H as KeyHasher>::Out) -> bool {
		if key == &self.hashed_null_node {
			return true;
		}
	...
	fn insert(&mut self, value: &[u8]) -> <M::H as KeyHasher>::Out {
		if value == M::NULL_VALUE {
			return self.hashed_null_node.clone();
		}
		let key = M::H::hash(value);
		match self.data.entry(key) {
			Entry::Occupied(mut entry) => {
	...
	fn emplace(&mut self, key:<M::H as KeyHasher>::Out, value: DBValue) {
		if &*value == M::NULL_VALUE {
			return;
		}
	...
	fn remove(&mut self, key: &<M::H as KeyHasher>::Out) {
		if key == &self.hashed_null_node {
			return;
		}
...
impl<M: MemoryDBConf> AsHashDB<M::H> for MemoryDB<M> {
	fn as_hashdb(&self) -> &HashDB<M::H> { self }
	fn as_hashdb_mut(&mut self) -> &mut HashDB<M::H> { self }
}
...
mod tests {
  extern crate rlp;
	...
	#[test]
	fn consolidate() {
		let mut main = MemoryDB::<KeccakHasher>::new(&NULL_RLP);
		let mut other = MemoryDB::<KeccakHasher>::new(&NULL_RLP);
